...
`v0.4.1` 已把功能闭环推进到真实数据预览；按既定范围不开发原生 App。默认无状态、
不持久化个人行程，未配置的商业来源会明确降级，不由大模型编造。

## 已实现

- 文本/短信、ICS 日历、PNG/JPEG 截图 OCR 和手工行程导入；
- 携程、航旅纵横、航司和日历来源识别，真实行程由用户导入并逐项确认；
- adsb.lol 当前航空器遥测、AviationWeather.gov 官方 METAR 和 Open-Meteo 未来预报；
- 官方交通通告的版本化时空匹配、固定主机 allowlist、TTL 缓存、重试与熔断；
- 航班时间窗、7 个机场的版本化流程、高德临近航班实时驾车 ETA/路况、机场公开天气和用户事件信号；
...
- 不自动预约、付款、退改签或接受平台协议；
- 高德实时路线仅在服务端 Key、用户同意、完整坐标和预计离家前 3 小时时间窗同时满足时启用；
- 高德未来路径规划属于企业高级服务，未获权前不会把当前路况冒充晚间预测；
- 当前高德 Key 仅限测试用途并限制阿里云服务器出口 IP；公开商业运营前需完成服务许可；
- 航司计划、值机和登机口只有在服务端配置获权接口后启用；
- ADS-B 只表示当前航空器遥测，不能冒充航司计划、延误或登机口；
...
交给外部测试者时使用 [0.4.1 测试说明](docs/TESTER_GUIDE.md)，真实数据调试证据见
[调试记录](docs/real_data_debug_report.md)。
