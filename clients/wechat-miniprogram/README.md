...
- 首页不再自动载入演示数据，未填写时只显示清晰的三步入口；
- 支持“出发地 → 已订航班通知 → 确认字段”的三步行程输入；
- 内置北京交通大学 → 首都机场 → 大连机场 → 大连理工大学一键验收行程；
- 建议页同时展示离校、到机场、起飞、落地、接驾和到校完整时间链；
...
5. 完成一键验收行程、短信、ICS、截图、提醒、地图二次确认、删除和失败路径验收；
6. 确认首页右上角显示“小程序 v0.4.2”；
7. Android 与 iPhone 各完成一次冷启动。
