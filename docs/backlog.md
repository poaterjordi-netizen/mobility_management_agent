...
已新增真实行程来源识别、公开 ADS-B、官方 METAR、出发地/机场天气、官方交通通告、来源
链接、缓存/熔断、安全绑定检查、真实联网诊断与外部测试手册。

0.4.1 进一步完成 `SRC-002` 的测试环境实现：独立高德 Web Service Key、服务器出口 IP
白名单、root-only 运行时注入、V5 推荐驾车 ETA/距离/TMC 路况、预计离家前 3 小时时间
语义门禁，以及北京望京公开测试点至首都机场 T3、晚间 PEK→DLC 行程案例。未来路径规划
高级服务、商业许可与真实试点校准仍属于生产晋级门禁。
...
- `SRC-002` 已具备受限测试 Key；商业/长期使用仍需服务许可，`SRC-004` 仍需合同接口；
